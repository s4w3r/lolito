...
//ADMINISTRADOR
 ...
 app.post('/eliminar-usuarios', async(req,res)=>{
    const Documento=req.session.Documento;
    const laco = await mysql.createConnection(db)
    const [IDuse] = await laco.execute('SELECT usuario.Codigo_u FROM usuario WHERE usuario.Documento = ?',[Documento])
    console.log(IDuse)

    try{
        await laco.execute('DELETE FROM usuario WHERE usuario.Codigo_u = ?',[IDuse[0].Codigo_u]);
        await laco.end();
    }
    catch(error){
        console.error('Error en el servidor',error)
...
app.post('/obtener-manzanas', async(req,res)=>{
    ...
    try{
    const laco = await mysql.createConnection(db);
    const[IDman] = await laco.execute('SELECT * FROM manzana')
    console.log(IDman)
    res.json({manzana: IDman.map(rwx=> ([rwx.Codigo_m, rwx.Nombre_m, rwx.Localidad, rwx.Direccion]))})
    await laco.end()
    }
